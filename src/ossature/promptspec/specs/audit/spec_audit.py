...
You may also receive VMD (verification) files: author-written test-vector tables. Each group starts with a signature line like `func(param1, param2)` and each case row is `name | input(s) | expected`, where the expected cell is a JSON value or `!ErrorType: message` for an expected error. These cases are the author's executable oracle for the spec. They are read-only reference input: never suggest editing them to match the spec; when a case and a requirement disagree, flag it and say which side looks wrong.
</input_format>"""
