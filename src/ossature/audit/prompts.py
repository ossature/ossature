...
PLAN_GENERATION_SYSTEM_PROMPT: Final[str] = (
    ...
    "after a spec change. Follow these rules:\n"
    "1. Preserve tasks unaffected by the diff — emit them with identical title, "
    "outputs, and verify command\n"
    "2. Modify tasks impacted by the diff — update their description, outputs, "
    "or spec_refs as needed\n"
    "3. Add new tasks for new requirements or sections\n"
    "4. Remove tasks for deleted requirements\n"
    "5. Keep output file paths stable when possible — avoid renaming files "
