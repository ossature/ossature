...
Within the changed spec, tasks the planner preserves carry over their build status and state. This means a typo fix or a minor tweak to one requirement won't throw away progress on the rest of the spec. Tasks the edit does affect reset to pending, even when their output files stay the same.
